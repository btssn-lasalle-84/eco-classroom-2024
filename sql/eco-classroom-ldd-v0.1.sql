...
DROP TABLE IF EXISTS EtatFenetres;
DROP TABLE IF EXISTS EtatLumieres;
DROP TABLE IF EXISTS EtatPresence;
...
CREATE TABLE IF NOT EXISTS `EtatLumieres` (
	`idEtatLumieres`	int AUTO_INCREMENT PRIMARY KEY,
	`idSalle`	int NOT NULL,
	`etatLumieres`	SMALLINT UNSIGNED NOT NULL,
	`horodatage`	DATETIME NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `EtatLumieres`
  ADD CONSTRAINT `EtatLumieres_fk_1` FOREIGN KEY (`idSalle`) REFERENCES `Salle` (`idSalle`) ON DELETE CASCADE;

-- --------------------------------------------------------

CREATE TABLE IF NOT EXISTS `EtatFenetres` (
	`idEtatFenetres`	int AUTO_INCREMENT PRIMARY KEY,
	`idSalle`	int NOT NULL,
...
ALTER TABLE `EtatFenetres`
  ADD CONSTRAINT `EtatFenetres_fk_1` FOREIGN KEY (`idSalle`) REFERENCES `Salle` (`idSalle`) ON DELETE CASCADE;
